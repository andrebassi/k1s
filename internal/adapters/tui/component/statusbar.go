...
func (s StatusBar) View() string {
	...
	return style.StatusBarStyle.Width(s.width).Render(
...
func (s StatusBar) renderLeft() string {
	...
	if s.context != "" {
		parts = append(parts, fmt.Sprintf("ctx:%s", style.StatusBarKeyStyle.Render(s.context)))
	}

	if s.namespace != "" {
		parts = append(parts, fmt.Sprintf("ns:%s", style.StatusBarKeyStyle.Render(s.namespace)))
	}

	if s.resource != "" {
		parts = append(parts, fmt.Sprintf("res:%s", style.StatusBarKeyStyle.Render(s.resource)))
	}
