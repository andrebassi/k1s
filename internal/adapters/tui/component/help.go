...
func (h HelpPanel) View() string {
	...
	for _, group := range h.entries {
		for _, entry := range group {
			key := style.HelpKeyStyle.Render(style.PadRight(entry.Key, 8))
			desc := style.HelpDescStyle.Render(entry.Desc)
			b.WriteString("  ")
			b.WriteString(key)
			b.WriteString(" ")
			b.WriteString(desc)
...
func (h HelpPanel) ShortHelp() string {
	...
	for _, s := range shortcuts {
		parts = append(parts,
			style.HelpKeyStyle.Render(s.Key)+
				style.HelpSeparator.Render(":")+
				style.HelpDescStyle.Render(s.Desc))
	}
