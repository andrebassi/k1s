...
func (m ManifestPanel) View() string {
	if !m.ready {
		return style.PanelStyle.Render("Loading manifest...")
	}
...
func (m ManifestPanel) renderHelpers() string {
	...
	b.WriteString(style.EventWarning.Render("Debug Hints\n"))
	for _, helper := range m.helpers {
		severity := style.StatusMuted
		switch helper.Severity {
		case "High":
			severity = style.StatusError
		case "Warning":
			severity = style.EventWarning
		}

		b.WriteString(severity.Render(fmt.Sprintf("  [%s] %s\n", helper.Severity, helper.Issue)))
		for _, suggestion := range helper.Suggestions {
			b.WriteString(style.SubtitleStyle.Render(fmt.Sprintf("    • %s\n", suggestion)))
		}
	}
...
func (m ManifestPanel) renderContainers() string {
	...
	b.WriteString(style.SubtitleStyle.Render("Containers\n"))
	for _, c := range m.pod.Containers {
		...
		b.WriteString(fmt.Sprintf("    Image:    %s\n", style.Truncate(c.Image, m.width-14)))
...
func (m ManifestPanel) renderRelated() string {
	...
	b.WriteString(style.SubtitleStyle.Render("Related Resources\n"))
...
func (m ManifestPanel) renderLabels() string {
	...
	b.WriteString(style.SubtitleStyle.Render("Labels\n"))
...
func (m ManifestPanel) renderContainerResources() string {
	...
	b.WriteString(style.SubtitleStyle.Render("Container Resources\n"))
	for _, c := range m.pod.Containers {
		b.WriteString(style.LogContainer.Render(fmt.Sprintf("  %s\n", c.Name)))
